...
class PlayerScreen : AppCompatActivity(), Player.Listener, AdManagerListener,SessionManagerListener<CastSession> {
            ...
            binding?.adViewTop?.let { it1 ->
                binding?.fbAdViewTop?.let { it2 ->
                    adManager?.loadAdProvider(
                        ...
                        it1, it2, binding?.unityBannerView, binding?.startAppBannerBottom
                    )
                }
            }
        }

        if (!Constants.location2BottomProvider.equals("none", true)) {
            binding?.adViewBottom?.let { it1 ->
                binding?.fbAdViewBottom?.let { it2 ->
                    adManager?.loadAdProvider(
                        Constants.location2BottomProvider, Constants.adLocation2bottom,
